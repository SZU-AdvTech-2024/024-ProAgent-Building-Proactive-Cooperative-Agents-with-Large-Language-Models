...
warnings.filterwarnings("ignore",
                        message=".*cuBLAS factory.*")  # ignore "Unable to register cuBLAS factory" due to use tf-CPU
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layout', '-l', type=str, default='cramped_room',
                        choices=['cramped_room', 'asymmetric_advantages', 'coordination_ring', 'forced_coordination',
                                 'counter_circuit'])
    parser.add_argument('--p0', type=str, default='Greedy',
                        choices=['ProAgent', 'Greedy', 'COLE', 'FCP', 'MEP', 'PBT', 'SP', 'BC', 'Random', 'Stay',
                                 'Human'], help='Algorithm for P0 agent 0')
    parser.add_argument('--p1', type=str, default='Greedy',
                        choices=['ProAgent', 'Greedy', 'COLE', 'FCP', 'MEP', 'PBT', 'SP', 'BC', 'Random', 'Stay',
                                 'Human'], help='Algorithm for P1 agent 1')
    ...
    parser.add_argument('--gpt_model', type=str, default='gpt-3.5-turbo-0301',
                        choices=['text-davinci-003', 'gpt-3.5-turbo-16k', 'gpt-3.5-turbo-0301', 'gpt-3.5-turbo',
                                 'gpt-4', 'gpt-4-0314'], help='Number of episodes')
    parser.add_argument('--prompt_level', '-pl', type=str, default='l2-ap', choices=['l1-p', 'l2-ap', 'l3-aip'],
    ...
    # variant = vars(args)
    variant = {
        'layout': 'cramped_room',  # 固定布局为 cramped_room
        'p0': 'Greedy',  # 玩家 0 使用 Greedy 策略
        'p1': 'Greedy',  # 玩家 1 使用 BC 策略
        'horizon': 400,  # 游戏步数为 400
        'episode': 1,  # 运行 1 个游戏回合
        'prompt_level': 'l2-ap',  # 提示工程级别为 l2-ap
        'belief_revision': False,  # 不使用信念修正
        'retrival_method': 'recent_k',  # 检索方式为最近 K 条
        'K': 1,  # 检索最近 1 条对话
        'mode': 'exp',  # 模式为逐步执行
        'save': True,  # 保存实验结果
        'log_dir': './logs',  # 日志保存目录
