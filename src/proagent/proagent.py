...
class ProMediumLevelAgent(ProAgent):
	...
	def create_gptmodule(self, module_name, file_type='txt', retrival_method='recent_k', K=6):
	...
	def get_lowest_cost_action_and_goal_new(self, start_pos_and_or, motion_goals, state): 
		...
		if best_action is None:
			if self.layout == 'cramped_room' and np.random.rand() < 0.5:
				return None, Action.STAY
			elif self.layout == 'asymmetric_advantages' and np.random.rand() < 0.4:
				return None, Action.STAY
			elif self.layout == 'coordination_ring' and np.random.rand() < 0.45:
				return None, Action.STAY
			elif self.layout == 'forced_coordination' and np.random.rand() < 0.45:
				return None, Action.STAY
			elif self.layout == 'counter_circuit' and np.random.rand() < 0.45:
				return None, Action.STAY
